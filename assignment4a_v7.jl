...
begin
	...
	using ImageView, ImageIO, FileIO, Images;
	using StatsBase;
...
begin
	...
	function change_fs(old_len, old_ys, old_fs :: Int, new_fs :: Int)
		...
		# # can't do perfect sinc because takes too long
		# new_ys = zeros(new_len)
		# for i=0:old_len-1
		# 	new_ys += old_ys[i+1] * sinc.(old_fs * (new_xs .- (i / old_fs)));
		# end

		fs_gcd = gcd(old_fs, new_fs);
		upsample_factor = div(new_fs, fs_gcd);
		downsample_factor = div(old_fs, fs_gcd);

		# TODO use these, check they aren't ridiculous

		dbg(upsample_factor);
		dbg(downsample_factor);
		
		# b = sinc.(old_fs * (new_xs .- (0.5 * new_len / old_fs))) .* Windows.hanning(new_len);
		# b /= sum(b);
		
		# dbg(length(old_ys));
		# dbg(length(b));
		# # num_zeros = length(b) - length(old_ys);
		# # dbg(num_zeros)
		# new_ys = conv(b, old_ys)[round(Int, length(b)/2 + 1):end]
		
		# # new_ys = filtfilt(b, [old_ys; zeros(num_zeros)]);
		# # new_ys = filtfilt(b, [old_ys; zeros(1000)])[1:new_len];
		# # new_ys = conv(b, new_ys[end:-1:1])[end:-1:1];
		# new_ys = new_ys[1:new_len];

		# new_ys = Windows.hanning(new_len);
		# # new_ys = b;
		# # new_xs = collect(0:length(new_ys)-1) / new_fs;
		# dbg(length(new_ys));
	...
	let
		...
		ref_fs = 1000;
		ref_len = round(Int, duration * ref_fs);
		ref_xs = collect(0:ref_len-1) / ref_fs;
		ref_ys = sin.(2π * ref_xs * freq);
	
		old_fs = 60;
		old_len = round(Int, duration * old_fs);
		old_xs = collect(0:old_len-1) / old_fs;
		old_ys = sin.(2π * old_xs * freq);
	
		new_fs = 70;
		_, new_xs, new_ys = change_fs(length(old_xs), old_ys, old_fs, new_fs)
		
		
		plot([ref_xs, old_xs, new_xs], [ref_ys, old_ys, new_ys], markershape=[:none :circle :x]);
	end
end
...
begin
	...
	xd, yd = 640, 480;
	xt, yt = 800, 525;
	fp_standard = Int(MHz(25.175)); # +-0.5% range
	fs = MHz(64);
end;
...
begin
	function plot_image(img :: AbstractMatrix; aspectratio=1 :: Real)
		heatmap(img, aspectratio=aspectratio, yflip=true, colorbar=:none, axis=([], false), color=cgrad(:grays));
	end;
...
let	
	...
	fr = 2 * fp; # TODO they imply other multiples might be better?

	new_num_samples = floor(Int, fr / fs * num_samples); # TODO the resample function uses ceiling I think? (should this be round or floor?)

	# TODO try other reinterpolation methods?
	# TODO probably need to lowpass filter it first

	# new_num_samples, _, new_samples = change_fs(num_samples, z, fs, new_fs);
	z_resampled = resample(z, fr/fs)[1:new_num_samples]; # TODO replace with own function later
	# dbg(new_num_samples);
	...
	frame1 = z_adjusted[samples_per_frame*0+1:samples_per_frame*1]
	frame5 = z_adjusted[samples_per_frame*4+1:samples_per_frame*5]
	test_img1 = reshape(abs.(frame1), (samples_per_line, :))';
	test_img2 = reshape(abs.(frame5), (samples_per_line, :))';
	dbg(size(test_img1));
	test_img = copy(test_img1);
	test_img[round(Int, 525/2):end, :] = test_img2[round(Int, 525/2):end, :]
	
	img_sz = (Int(samples_per_line/samples_per_pixel), 
		Int(length(test_img)/samples_per_line)); # this is just (800, 525)
	# imshow(test_img; aspect=:none, canvassize=img_sz); # DEBUG
	
	# no idea what the deal is with the darker/whiter regions in the blanking intervals, but I seem to have done the alignment correctly
	# imshow(test_img[1:480, 1:640*samples_per_pixel]; aspect=:none, canvassize=(640, 480));

	# # fix aspect ratio and save image to disk
	# test_img2 = reshape(sum(reshape(test_img, (525, 2, 800)), dims=2), (525, 800))/2;
	# test_img2 = test_img2 .- findmin(test_img2)[1];
	# test_img2 = test_img2 / findmax(test_img2)[1];
	# save("test.png", test_img2);
...
begin
	xs_fh, xs_fv, autocor_fh, autocor_fv = let
		z = iq_samples;
		...
		# trying to get exact estimates for fp (and therefore also fh, fv, etc.)
		# here we use estimates to get the approx number of samples in a few frames
		est_samples_per_pixel = round(Int, fs/fp_std);
		est_samples_per_line = round(Int, fs/fh_std);
		est_samples_per_frame = round(Int, fs/fv_std);
	
		autocor_samples = 2 * est_samples_per_frame;
		
		xs_fh = floor(Int, fs/fh_high):ceil(Int, fs/fh_low);
		xs_fv = floor(Int, fs/fv_high):ceil(Int, fs/fv_low);
		# plot(xcorr(abs.(z[1:autocor_samples]), abs.(z[1:autocor_samples])));
		# plot(xs, autocor(abs.(z), xs); xticks=[fs/ref_fh]);
		# plot(autocor(abs.(z[1:autocor_samples]), 0:round(Int, 1.2* est_samples_per_frame)); xticks=[fs/fv]);
	
		autocor_fh = autocor(abs.(z), xs_fh);
		autocor_fv = autocor(abs.(z), xs_fv); # SLOW

		dbg(xs_fh[findmax(autocor_fh)[2]])
		
		# TODO simulate noisier conditions?

		xs_fh, xs_fv, autocor_fh, autocor_fv; # export
	end;
end;

# ╔═╡ 51604c61-b924-451e-84e2-c751473c5ff8
begin
	autocor_fh_short = let
		z = iq_samples;
		
		num_frames = 2;
		num_frames_total = fv_std * length(z) / fs; # estimated
		num_samples = round(Int, length(z) * num_frames / num_frames_total);
		autocor(abs.(z[1:num_samples]), xs_fh);
	end;
end;

# ╔═╡ 91620a51-b3fe-4f24-ac5e-2bc65502b90b
let
	plot(xs_fh, autocor_fh_short; xticks=([fs/fh_ref, fs/fh_std], ["tuned", "untuned"]), xrotation=90, tickfontsize=4, dpi=300, label="autocorrelation (fh)");
end

# ╔═╡ fc616bd8-1f00-47c0-ba47-d021375d5345
let
	...
	dbg(typeof(autocor_fh_filtered));
	
	# plot(xs_fh, autocor_fh; xticks=([fs/fh_ref, fs/fh_std], ["tuned", "untuned"]), xrotation=90, tickfontsize=4, dpi=300, label="autocorrelation (fh)");
	plot(xs_fh, autocor_fh_filtered; xticks=([fs/fh_ref, fs/fh_std], ["tuned", "untuned"]), xrotation=90, tickfontsize=4, dpi=300, label="autocorrelation (fh)");
end

# ╔═╡ 79c135a5-2164-4fa8-bbdc-b4b0b36e99cb
begin
	autocor_fv_short = let
		z = iq_samples;
		
		num_frames = 4;
		num_frames_total = fv_std * length(z) / fs; # estimated
		num_samples = round(Int, length(z) * num_frames / num_frames_total);
		autocor(abs.(z[1:num_samples]), xs_fv);
	end;
end;

# ╔═╡ 221a1c69-70f4-484d-8d17-07b110cee94b
let
	plot(xs_fv, autocor_fv_short; xticks=([fs/fv_ref, fs/fv_std], ["tuned", "untuned"]), xrotation=90, tickfontsize=4, dpi=300, label="autocorrelation (fv)");
end

# ╔═╡ 556902cb-cf1a-4c11-9541-4a0e7603b787
let
	ker = ImageFiltering.Kernel.gaussian((400,));
	autocor_fv_filtered = imfilter(autocor_fv, ker);

	# TODO not sure about effectiveness of gaussian (how do we pick standard dev?)
	println(xs_fv[findmax(autocor_fv)[2]]);
	println(xs_fv[findmax(autocor_fv_filtered)[2]]);
	
	# plot(xs_fv, autocor_fv_filtered; xticks=([fs/fv_ref, fs/fv_std], ["reference", "default"]), xrotation=90, tickfontsize=4);
	plot(xs_fv, autocor_fv; xticks=([fs/fv_ref, fs/fv_std], ["tuned", "untuned"]), xrotation=90, tickfontsize=4, dpi=300, label="autocorrelation (fv)");
	
	# TODO compare the plot to if you calculated with much fewer samples

	# TODO cross correlation?
	# TODO taking the autocorrelation of the complex samples, before taking absolute value? (Maybe just do xs_fh)
	# TODO I know why there's a center spike, why all the other ones?
...
begin
	xcorr_fv = let # TODO try xcorr_fh as well
		# TODO use autocor between frames
	end
end
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "2e1a0e8a779b5b1138c1d63481e699967cbfacb7"
...
git-tree-sha1 = "43f85714a2e853154ceeb322af688c2f17fffd14"
