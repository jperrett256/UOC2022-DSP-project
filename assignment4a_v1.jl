...
begin
	...
	new_num_samples = round(Int, fr / fs * num_samples); # TODO the resample function uses ceiling (should this be round or floor?)
	...
	println(test_img);
	# closeall();
	img_sz = (Int(samples_per_line/(fr/fp)), Int(length(test_img)/samples_per_line));
	dbg(typeof(sz));
	imshow(test_img; aspect=:none, canvassize=img_sz); # TODO set aspect ratio to fr/fp (= 2:1)
	# heatmap(test_img, aspectratio=fr/fp, yflip=true, xlims=(0,samples_per_line))

	# TODO skip unnecessary samples manually
end
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "d7ecad2be783aa00b326aac512fcee074ccd212e"
