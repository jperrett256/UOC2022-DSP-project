...
begin
	function plot_image(img :: AbstractMatrix; aspectratio=1 :: Real)
		heatmap(img, aspectratio=aspectratio, yflip=true, xticks=:none, yticks=:none, colorbar=:none, color=cgrad(:grays));
	end
end
...
begin
	...
	xd, yd = 640, 480;
	xt, yt = 800, 525;
	fp = Int(MHz(25.175)) * 1.001; # TODO +-0.5% range
	fs = MHz(64);
	fh = fp/xt;
	fv = fh/yt;
	...
	new_num_samples = floor(Int, fr / fs * num_samples); # TODO the resample function uses ceiling (should this be round or floor?)

	# TODO coudl try reinterpolating using non-causual perfect sinc?
	...
	z_resampled = z_resampled[start_idx:start_idx+samples_per_frame-1];
	# dbg(length(z_resampled));
	

	test_img = reshape(abs.(z_resampled), (samples_per_line, :))';
	dbg(typeof(test_img))
	# dbg(samples_per_line);
	# println(abs.(z_resampled[1:100]));
	# println(abs.(z_resampled[1601:1700]));
	# println(test_img);
	# closeall();
	img_sz = (Int(samples_per_line/samples_per_pixel), 
		Int(length(test_img)/samples_per_line)); # this is just (800, 525)
	imshow(test_img; aspect=:none, canvassize=img_sz);
	...
	# heatmap(test_img, aspectratio=fr/fp, yflip=true, xlims=(0,samples_per_line), xticks=:none, yticks=:none, colorbar=:none, color=cgrad(:grays))

	# TODO figure out how to save images to disk - probably sth reshape related
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "ba2e718fd9e692449c18c337ad67e8b6fcdceebb"
