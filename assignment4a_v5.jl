...
# ╔═╡ c6ce42df-c4d8-45fd-aa91-f8e984ddf955
begin
...
begin
	...
	xd, yd = 640, 480;
	xt, yt = 800, 525;
	fp = Int(MHz(25.175)) * 1.000997; # TODO +-0.5% range
	fs = MHz(64);
	fh = fp/xt;
	fv = fh/yt;

	scene_fn = "scene3-640x480-60-425M-64M-40M.dat";
	num_samples_total = div(filesize(scene_fn), sizeof(ComplexF32));
	num_frames_total = fv * num_samples_total / fs;

	num_frames = 8;
	num_samples = round(Int, num_samples_total * num_frames / num_frames_total); # get first couple frames
	...
	# TODO could try reinterpolating using non-causual perfect sinc?
	...
	# TODO display half of two frames joined in the middle so you can do vertical alignment
	# TODO automatically deducing these characteristics
	# autocor(frame1)
	# plot(abs.(xcorr(
	# 	z_resampled[dbg_idx:dbg_idx+4000-1], 
	# 	z_resampled[dbg_idx:dbg_idx+4000-1])));
end

# ╔═╡ 821219b2-44bb-43a1-b442-089d75a64a6a
let
	num_samples = samples_per_frame * 2;
	z_subrange = z_resampled[1:num_samples];

	num_samples_half = ceil(Int, num_samples/2);
	xs = (collect(1:num_samples) .- num_samples_half) / num_samples_half * fr/2;
	# ys = fft(z_subrange) .* fft(conj.(z_subrange));
	# plot(xs, abs.(fftshift(ys)));
	# plot(xs, abs.(ifft(ys)));
end
...
let
	...
	ref_fp = round(Int, MHz(25.175) * 1.000997); # TODO +-0.5% range
	ref_fh, ref_fv = get_freqs(ref_fp);
	# dbg(fs/ref_fh);
	# dbg(fs/ref_fv);

	low_fp = floor(Int, MHz(25.175) * 0.95);
	low_fh, low_fv = get_freqs(low_fp);
	high_fp = ceil(Int, MHz(25.175) * 1.05);
	high_fh, high_fv = get_freqs(high_fp);
	# dbg(fs/low_fh);
	# dbg(fs/high_fh);

	def_fh, def_fv = get_freqs(Int(MHz(25.175)));
	...
	est_samples_per_pixel = round(Int, fs/fp);
	est_samples_per_line = round(Int, fs/fh);
	est_samples_per_frame = round(Int, fs/fv);

	autocor_samples = 2 * est_samples_per_frame;
	# dbg(autocor_samples);
	xs_fh = floor(Int, fs/high_fh):ceil(Int, fs/low_fh);
	xs_fv = floor(Int, fs/high_fv):ceil(Int, fs/low_fv);
	...
	dbg(floor(Int, fs/high_fh)+findmax(autocor_fh_filtered)[2]);

	autocor_fv = autocor(abs.(z), xs_fv);
	# TODO try taking gaussian of 
	
	# plot(xs_fh, autocor_fh_filtered; xticks=([fs/ref_fh, fs/def_fh], ["reference", "default"]), xrotation=90, tickfontsize=4);
	plot(xs_fv, autocor_fv; xticks=([fs/ref_fv, fs/def_fv], ["reference", "default"]), xrotation=90, tickfontsize=4);

	# TODO fix resolution of xs_fv plot
	# TODO first just separate computing results and plotting them into separate blocks in this notebook (maybe refactor everything?)
	
	# TODO simulate noisier conditions?
end

# ╔═╡ 58884d55-3769-45c7-a642-eb50bb2b65df
begin
	x =
	let
		y = 10;
